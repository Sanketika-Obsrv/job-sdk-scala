...
import org.sunbird.obsrv.job.model.Producer.Producer
import org.sunbird.obsrv.job.model.StatusCode.StatusCode
import org.sunbird.obsrv.job.model.ErrorConstants.Error
import org.sunbird.obsrv.job.model.{Constants, Stats, StatusCode}
...
trait JobMetrics {
  def registerMetrics(datasets: List[String], metrics: List[String]): Metrics = {
    ...
    val allDatasets = datasets ++ List( "ALL")
